...
interface VkService {
    ...
    @GET("users.get?fields=domain,first_name,last_name,photo_max,about,bdate,city,country,career,education,followers_count,last_seen&v=5.131")
    fun getProfile(): Single<ProfileResponse>

    @GET("wall.get?extended=1&count=10&v=5.131")
    fun getWall(
        @Query("offset") offset: Int
    ): Single<WallResponse>

    @GET("wall.post?extended=1&count=10&v=5.131")
    fun post(
    ...
    @GET("newsfeed.get?filters=post&need_likes=1&count=10&v=5.131")
    fun getNews(
        @Query("start_from") start: Int
    ): Single<ResponseNewsfeed>

    @GET("wall.getComments?filters=post&v=5.131&count=10&preview_length=0&extended=1&need_likes=1")
    fun getComments(
    ...
    @GET("wall.getById?v=5.131&extended=1")
    fun getPostById(
        @Query("posts") id: String
    ): Single<ResponseNewsfeed>

    @GET("access_token?v=5.131&client_id=7638442&client_secret=n981thkA7iLi9qOiIRPi&grant_type=client_credentials")
    fun serviceKey(): Single<ServiceKey>

    @GET("secure.checkToken?v=5.131&client_secret=n981thkA7iLi9qOiIRPi")
    fun checkToken(
        @Query("token") token: String,
        @Query("access_token") access_token: String
    ): Single<CheckToken>

    @GET("photos.getWallUploadServer?v=5.131")
    ...
    @GET("photos.saveWallPhoto?v=5.131")
    fun saveWallPhoto(
    ...
    @GET("docs.getWallUploadServer?v=5.131")
    fun getDocsUploadServer(): Single<ResponseUploadUrl>

    @GET("docs.save?v=5.131")
    fun saveWallDocs(
        @Query("file") file: String
    ): Single<SaveWallDocs>

    @GET("wall.createComment?v=5.131")
    fun createComment(
    ...
    @GET("database.getCities?need_all=1&count=1000&v=5.131")
    fun getCountriesById(
        @Query("country_ids") country_ids: Int
    ): Single<GetCountriesById>

    @GET("database.getCitiesById?v=5.131")
    fun getCities(
